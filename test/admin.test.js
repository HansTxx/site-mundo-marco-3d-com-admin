...
test('fluxo HTTP: login, cookies, pedidos protegidos, validação, duplicação e logout', async t => {
  ...
  const payload = { cliente: { nome: '<img src=x onerror=alert(1)>', email: 'teste@example.com', telefone: '(47) 99999-9999',
  ...
  for (const result of results) {
    assert.equal(result.status, 201); const data = await result.json(); assert.equal(data.numero, 1); assert.match(data.mensagem, /TOTAL:\* R\$\s218,70/);
  }
  const orders = await (await request('/api/admin/pedidos', null, auth)).json();
  assert.equal(orders.pedidos.length, 1); assert.equal(orders.pedidos[0].subtotal, 199.8);
  ...
  assert.equal(managed.total, 37.02); assert.equal(managed.observacoes, edit.observacoes);
