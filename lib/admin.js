...
module.exports = function installAdmin(app, productStore) {
  const store = new OrderStore(process.env.DATA_DIR || path.join(__dirname, '..', 'data'));
  const accounts = new (require('./accounts'))(process.env.DATA_DIR || path.join(__dirname, '..', 'data'));
  ...
  async function authorized(req, res, next) {
    ...
    try {
      if (!await accounts.valid(session)) { sessions.delete(value); return res.status(401).json({ erro: 'Seu acesso foi alterado. Entre novamente.' }); }
      req.adminSession = session;
      next();
    } catch (error) { res.status(503).json({ erro: 'Não foi possível verificar o acesso. Verifique o servidor.' }); }
  }
  ...
  function limit(map, maximum, windowMs, onlyFailures = false) {
    return (req, res, next) => {
      const key = req.ip;
      let entry = map.get(key);
      if (!entry || entry.until <= Date.now()) {
        if (map.size >= 10000) return res.status(429).json({ erro: 'Tente novamente mais tarde.' });
        entry = { count: 0, pending: 0, until: Date.now() + windowMs }; map.set(key, entry);
      }
      if (onlyFailures ? entry.count + entry.pending >= maximum : ++entry.count > maximum) {
      ...
      if (onlyFailures) {
        // Reserve in-flight checks so parallel requests cannot bypass the cap.
        entry.pending++;
        let released = false;
        const release = () => { if (!released) { released = true; entry.pending--; } };
        ...
        res.once('finish', () => {
          ...
          if (res.statusCode === 200) {
            ...
            // Start the window at the first rejected login, not an earlier success.
            if (entry.count === 0) entry.until = Date.now() + windowMs;
            entry.count++;
          }
        });
      }
  ...
  const cleanup = setInterval(() => {
    ...
    for (const map of [attempts, orderAttempts, accountAttempts]) for (const [key, v] of map) if (v.until <= Date.now()) map.delete(key);
  }, 60000);
  cleanup.unref();
  ...
  app.post('/api/admin/login', sameOrigin, limit(attempts, 10, 5 * 60000, true), async (req, res) => {
    if (secure && !req.secure) return res.status(400).json({ erro: 'O acesso administrativo em produção exige HTTPS.' });
    let identity;
    try { identity = await accounts.authenticate(req.body?.usuario, req.body?.senha); }
    catch { return res.status(503).json({ erro: 'Não foi possível carregar o acesso. Verifique a configuração e o arquivo privado no servidor.' }); }
    if (!identity) return res.status(401).json({ erro: 'Usuário ou senha incorretos.' });
    ...
    sessions.set(value, { expires: Date.now() + lifetime, ...identity });
  ...
  app.get('/api/admin/conta', authorized, async (req, res) => {
    try {
      const session = req.adminSession;
      const admin = session.role === 'master' ? await accounts.read() : null;
      res.json({ role: session.role, usuario: session.usuario, adminUsuario: session.role === 'master' ? (admin?.usuario || '') : session.usuario });
    } catch { res.status(503).json({ erro: 'Não foi possível ler o acesso administrativo.' }); }
  });
  app.post('/api/admin/conta', authorized, sameOrigin, limit(accountAttempts, 10, 15 * 60000), async (req, res) => {
    try {
      await accounts.change(req.adminSession, req.body || {});
      for (const [key, session] of sessions) if (session.role === 'admin') sessions.delete(key);
      res.json({ ok: true, entrarNovamente: req.adminSession.role === 'admin' });
    } catch (error) { res.status(error.status || 503).json({ erro: error.status ? error.message : 'Não foi possível salvar o acesso. Tente novamente.' }); }
  });
  app.get('/admin/products.js', securityHeaders, (req, res) => res.sendFile(path.join(__dirname, '..', 'admin', 'products.js')));
  app.get('/api/admin/produtos', authorized, async (req, res) => {
    try { res.json(await productStore.list()); } catch { res.status(503).json({ erro: 'Não foi possível ler o catálogo. Verifique o arquivo e o backup.' }); }
  });
  const saveProduct = remove => async (req, res) => {
    try {
      const id = req.params.id === undefined ? undefined : Number(req.params.id);
      const product = await productStore.save(req.body || {}, id, remove);
      res.status(id === undefined ? 201 : 200).json(product);
    } catch (error) { res.status(error.status || 503).json({ erro: error.status ? error.message : 'Não foi possível salvar o catálogo. Verifique o armazenamento do servidor.' }); }
  };
  app.post('/api/admin/produtos', authorized, sameOrigin, saveProduct(false));
  app.post('/api/admin/produtos/:id', authorized, sameOrigin, saveProduct(false));
  app.post('/api/admin/produtos/:id/excluir', authorized, sameOrigin, saveProduct(true));
  app.post('/api/admin/imagens', authorized, sameOrigin, async (req, res) => {
    try {
      const value = req.body?.imagem;
      if (typeof value !== 'string') return res.status(400).json({ erro: 'Envie uma imagem PNG, JPG, WEBP ou GIF.' });
      ...
      if (!type || type !== match[1] || bytes.length > 4 * 1024 * 1024) return res.status(400).json({ erro: 'Imagem inválida ou maior que 4 MB.' });
      const fs = require('node:fs/promises');
      const directory = path.join(process.env.DATA_DIR || path.join(__dirname, '..', 'data'), 'imagens');
      await fs.mkdir(directory, { recursive: true, mode: 0o700 });
      const name = crypto.createHash('sha256').update(bytes).digest('hex') + '.' + (type === 'jpeg' ? 'jpg' : type);
      await productStore.atomicWrite(path.join(directory, name), bytes);
      res.status(201).json({ imagem: '/media/' + name });
    } catch { res.status(503).json({ erro: 'Não foi possível salvar a imagem.' }); }
  });
  ...
  function validate(body, admin = false, catalog = []) {
    ...
    const itens = body.itens.map(item => {
      if (admin) {
        ...
        return { id: typeof item.id === 'number' && Number.isSafeInteger(item.id) ? item.id : null,
          variante: text(item.variante ?? '', 'modelo', 100, false), nome: text(item.nome, 'nome do produto', 420), quantidade: item.quantidade, preco,
          subtotal: Math.round(preco * 100) * item.quantidade / 100 };
      }
      const p = catalog.find(p => p.id === item?.id);
      if (!p || seen.has(JSON.stringify([p.id, item.variante || ""])) || !Number.isInteger(item.quantidade) || item.quantidade < 1 || item.quantidade > 999) throw new Error('Produto ou quantidade inválida.');
      const variante = item.variante ?? '';
      if (typeof variante !== 'string' || ((p.variantes || []).length ? !p.variantes.some(v => v.nome === variante) : variante !== '')) throw new Error('Selecione um modelo disponível para o produto.');
      seen.add(JSON.stringify([p.id, variante]));
      return { id: p.id, variante, nome: p.nome + (variante ? ` — Modelo: ${variante}` : ''), quantidade: item.quantidade, preco: p.preco,
    ...
    const extra = {};
    if (!admin || body.personalizacao !== undefined) {
      if (body.personalizacao !== undefined && (!body.personalizacao || typeof body.personalizacao !== 'object' || Array.isArray(body.personalizacao))) throw new Error('Observações do cliente inválidas.');
      extra.personalizacao = {};
      for (const [key, label] of Object.entries({ corSuporte: 'Cor - Suporte da munição', corTampa: 'Cor/Personalização - Tampa da caixa', corTrava: 'Cor - Trava' })) {
        extra.personalizacao[key] = text(body.personalizacao?.[key] ?? '', label, 1000, false);
      }
    }
    return { cliente, itens, subtotal, frete, total: Math.round((subtotal + frete.valor) * 100) / 100, ...extra };
  }
  ...
  app.post('/api/pedidos', sameOrigin, limit(orderAttempts, 30, 60000), async (req, res) => {
    ...
    try {
      ...
      order = validate(req.body, false, await productStore.list());
    } catch (error) { return res.status(400).json({ erro: error.message }); }
    try {
      const saved = await store.add(order, key, digest(JSON.stringify(order)).toString('hex'));
      const money = value => value.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' });
      const c = saved.cliente;
      const personalizacao = saved.personalizacao || {};
      const observacoesCliente = ['*OBSERVAÇÕES DO CLIENTE*',
        `Cor - Suporte da munição: ${personalizacao.corSuporte || 'Não informado'}`,
        `Cor/Personalização - Tampa da caixa: ${personalizacao.corTampa || 'Não informado'}`,
        `Cor - Trava: ${personalizacao.corTrava || 'Não informado'}`].join('\n');
      ...
      res.status(201).json({ numero: saved.numero, dataHora: saved.dataHora,
